...
def read_master_salt_strict(username: str) -> bytes:
    """Read master salt from the identity header only.

    This is the strict path for new code. Legacy .slt files are ignored here so
    callers do not silently keep depending on the old storage model.
    """
    salt = _header_get_salt(username)
    if not salt:
        raise FileNotFoundError(f"Master salt missing from identity header for user={username}")
    if len(salt) != EXPECTED_SALT_LEN:
        raise ValueError(
            f"Invalid master salt length in identity header: {len(salt)} (expected {EXPECTED_SALT_LEN})"
        )
    return salt


def load_master_salt_for_user(username: str) -> bytes:
    """Compatibility alias for identity-header salt loading."""
    return read_master_salt_strict(username)


def delete_legacy_salt_file(username: str) -> bool:
    """Best-effort cleanup of the old standalone salt file."""
    try:
        sp = _legacy_salt_path(username)
        if not sp.exists():
            return False
        sp.unlink()
        log.info("[SALT] deleted legacy salt file: %s", sp)
        return True
    except Exception as e:
        log.debug("[SALT] delete legacy salt skipped/failed for %s: %r", username, e)
        return False
